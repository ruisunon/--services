...
package com.rxdata.catalogservice.services;

import com.rxdata.catalogservice.entities.Product;
import com.rxdata.catalogservice.exception.ProductNotFoundException;
import com.rxdata.catalogservice.mapper.ProductMapper;
import com.rxdata.catalogservice.model.response.InventoryDto;
import com.rxdata.catalogservice.model.response.PagedResult;
import com.rxdata.catalogservice.repositories.ProductRepository;
...
public class ProductService {
    ...
    public PagedResult<Product> findAllProducts(int pageNo, int pageSize, String sortBy, String sortDir) {
        ...
        if (!productCodeList.isEmpty()) {
            Map<String, Integer> inventoriesMap = getInventoryByProductCodes(productCodeList).stream()
                            .collect(Collectors.toMap(InventoryDto::productCode, InventoryDto::availableQuantity));
            productPage.getContent().forEach(product -> product.setInStock(
    ...
    public Product findProductById(Long id) {
        return findProductByProductId(id).map(
                        product -> {
